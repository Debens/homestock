...
describe('mapInputKeys', () => {
    ...
    const mapKeys: (keyof WrappedFieldInputProps)[] = ['onChange', 'name'];

    const mapInputToProps = mapInputKeys<IComponentInputProps>('onChange', 'name');
    const inputProps: any = mapInputToProps(input);
    ...
    it('should not include extraneous keys', () => {
        Object.keys(input).forEach(key => {
            if (!List(mapKeys).includes(key as keyof WrappedFieldInputProps)) {
